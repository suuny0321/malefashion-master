/* 2024-03-21 반응형 사이즈 조정 및 글 내용 정리 */
$(document).ready(function() {
	...
	const updateCount = () => {
		...
		if (count > 1) {
			document.getElementById("teamCount").innerHTML = `(${count}명)`;
		} else if (count == 1) {
			document.getElementById("teamCount").innerHTML = `(${count}명)`;
		} else {
			document.getElementById("teamCount").innerHTML = `(아무도 없습니다.)`;
		}
	};
	...
	const emptyState = (x) => {
		...
		newEmptyState.innerHTML = `
			...
			<h3 class="u-text--regular">아무도 없습니다.</h3>
			<p>채팅방에 인원을 초대해주세요.</p><br/>
		`;
	...
	const sendForm = () => {
		...
		if (!nameInput.value.length == 0 && !positionInput.value.length == 0) {
			...
			<div class="c-contact__left">
				<div class="c-contact__avatar" style="background-color: var(--${states[randomState]
				})">${nameInput.value.charAt(0)}</div>
			...
			<div class="l-actions c-contact__right">
				<div class="c-button c-button--danger c-button--sm c-button--delete">삭제</div>
				<div class="c-button c-button--default c-button--sm c-button--view">프로필</div>
			</div>
		`;
			...
			createAlert(
				...
				"success",
				`${nameInput.value} 님을 초대하였습니다.`
			);
			newItem
				.querySelector(".c-button--delete")
			...
			updateCount();
			if (
				document.querySelector(".u-hide--mobile.is-open") 
				//document.querySelector(".c-overlay")
			) {
				closeMobileForm();
	...
	document.getElementById("closeMobileAdd").addEventListener("click", () => {
		if (
			document.querySelector(".u-hide--mobile.is-open") //&& document.querySelector(".c-overlay")
		) {
			closeMobileForm();
			// document.querySelector(".c-overlay").remove();
		}
